...
<x-app-layout>
  ...
  <div class="mx-4 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
    <form method="GET" action="{{ route('product.create') }}">
      ...
      <x-primary-button class="mt-4 hover:bg-white focus-visible:bg-white active:bg-white focus:bg-white focus-within:bg-white">{{ __('Create Product') }}</x-primary-button>
    </form>
  </div>
      ...
      <div>
        <div class="flex gap-3 mt-4">
          ...
          <form method="GET" action="{{ route('product.show', $product) }}" class=" inline-block w-full m-0">
            @csrf
            <x-input-error :messages="$errors->get('message')" class="mt-2" />
            <x-primary-button class="justify-center w-full hover:bg-white focus-visible:bg-white active:bg-white focus:bg-white focus-within:bg-white">{{ __('View') }}</x-primary-button>
          </form>
          @if(Auth::user()->role != "Customer")
          <form method="GET" action="{{ route('product.edit', $product) }}" class="inline-block w-full m-0">
            @csrf
            <x-primary-button class="justify-center w-full hover:bg-white focus-visible:bg-white active:bg-white focus:bg-white focus-within:bg-white">
              {{ __('Edit') }}
            </x-primary-button>
          </form>
          @endif
        ...
        <form method="POST" action="{{ route('product.destroy', $product) }}" class=" inline-block w-full m-0 mt-3">
          ...
          @method('DELETE')
          <x-input-error :messages="$errors->get('message')" class="mt-2" />
          <x-delete-button class="justify-center w-full dark:bg-red-600 dark:text-white hover:bg-red-400 dark:focus-visible:bg-red-400 bg-red-600 text-white focus-visible:bg-red-400 del-btn focus:bg-red-400">{{ __('Delete') }}</x-delete-button>
        </form>
        @endif
      </div>
    </div>
